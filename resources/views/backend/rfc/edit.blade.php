...
@section('content')
    ...
    <div class="row">
      <div class="col-12">
        <div class="card my-4">
          <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
            <div class="bg-gradient-dark shadow-dark border-radius-lg pt-4 pb-3">
              <h6 class="text-white text-capitalize ps-3">Edit RFC</h6>
            </div>
          </div>
        ...
        @endif
            <form action="{{route('update.rfc', $data->id)}}" method="POST" enctype="multipart/form-data" style="padding-left: 20px; padding-right: 20px;">
                @csrf
                @method('PUT')
                <div class="mb-3">
                  <label for="title" class="form-label">Name</label><br>
                  <input type="text" id="title" name="title" aria-describedby="title" class="form-control form-control-pill" value="{{ old('title', $data->title) }}" required>
                </div>
